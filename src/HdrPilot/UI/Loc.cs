...
internal static class Loc
{
    ...
    private static readonly Dictionary<string, string> En = new()
    {
        ...
        ["entry.autoHdr.hint"] = "Upgrades this SDR game to HDR via the Windows graphics settings (requires the path). If Auto HDR is off globally, HDR Pilot turns it on while the game is running and restores it afterwards. Not recommended together with NVIDIA RTX HDR – both tone-map the image and will conflict.",
    ...
    private static readonly Dictionary<string, string> De = new()
    {
        ...
        ["entry.autoHdr.hint"] = "Wertet dieses SDR-Spiel über die Windows-Grafikeinstellungen zu HDR auf (Pfad erforderlich). Ist Auto-HDR global aus, schaltet HDR Pilot es für die Dauer der Spielsession ein und stellt es danach wieder her. Nicht zusammen mit NVIDIA RTX HDR empfohlen – beide wenden Tone-Mapping an und geraten in Konflikt.",
    ...
    private static readonly Dictionary<string, string> Fr = new()
    {
        ...
        ["entry.autoHdr.hint"] = "Convertit ce jeu SDR en HDR via les paramètres graphiques de Windows (chemin requis). Si Auto HDR est désactivé globalement, HDR Pilot l'active pendant la session de jeu puis restaure l'état précédent. Déconseillé avec NVIDIA RTX HDR – les deux appliquent un tone mapping et entrent en conflit.",
    ...
    private static readonly Dictionary<string, string> Es = new()
    {
        ...
        ["entry.autoHdr.hint"] = "Convierte este juego SDR a HDR mediante la configuración gráfica de Windows (requiere la ruta). Si Auto HDR está desactivado globalmente, HDR Pilot lo activa durante la sesión de juego y luego restaura el estado anterior. No se recomienda junto con NVIDIA RTX HDR: ambos aplican tone mapping y entran en conflicto.",
