...
internal static class Loc
{
    ...
    private static readonly Dictionary<string, string> En = new()
    {
        ...
        ["wl.btn.addFile"] = "Choose .exe file…",
        ["wl.btn.addRunning"] = "Running program…",
        ["wl.btn.edit"] = "Edit…",
        ...
        ["entry.sec.hdrEnhance"] = "HDR enhancement (SDR games)",
        ["entry.hdrEnhance.off"] = "None",
        ["entry.hdrEnhance.autoHdr"] = "Windows Auto HDR",
        ["entry.hdrEnhance.rtx"] = "NVIDIA RTX HDR",
        ["entry.hdrEnhance.hint"] = "Upgrades SDR games to HDR. Auto HDR is set in the Windows graphics settings (requires the path); if Auto HDR is off globally, HDR Pilot turns it on while the game is running and restores it afterwards. RTX HDR is set in the game's NVIDIA driver profile (requires an RTX GPU and a one-time administrator confirmation when saving); fine-tuning stays in the NVIDIA app.",
        ["entry.rtxHdr.noNvidiaTitle"] = "NVIDIA driver not found",
        ["entry.rtxHdr.noNvidiaMsg"] = "The NVIDIA driver interface (nvapi) is not available on this system. The RTX HDR setting will likely have no effect.",
        ["rtx.applyFailedTitle"] = "RTX HDR not applied",
        ["rtx.applyFailedMsg"] = "The RTX HDR change was not written to the NVIDIA driver profile. Writing it requires administrator rights – please confirm the Windows prompt when saving the list.",
    ...
    private static readonly Dictionary<string, string> De = new()
    {
        ...
        ["wl.btn.addFile"] = "EXE-Datei wählen…",
        ["wl.btn.addRunning"] = "Laufendes Programm…",
        ...
        ["entry.sec.hdrEnhance"] = "HDR-Verbesserung (SDR-Spiele)",
        ["entry.hdrEnhance.off"] = "Keine",
        ["entry.hdrEnhance.autoHdr"] = "Windows Auto-HDR",
        ["entry.hdrEnhance.rtx"] = "NVIDIA RTX HDR",
        ["entry.hdrEnhance.hint"] = "Wertet SDR-Spiele zu HDR auf. Auto-HDR wird in den Windows-Grafikeinstellungen gesetzt (Pfad erforderlich); ist Auto-HDR global aus, schaltet HDR Pilot es für die Dauer der Spielsession ein und stellt es danach wieder her. RTX HDR wird im NVIDIA-Treiberprofil des Spiels gesetzt (RTX-GPU erforderlich, beim Speichern einmalige Administrator-Bestätigung); die Feinabstimmung bleibt in der NVIDIA App.",
        ["entry.rtxHdr.noNvidiaTitle"] = "NVIDIA-Treiber nicht gefunden",
        ["entry.rtxHdr.noNvidiaMsg"] = "Die NVIDIA-Treiberschnittstelle (nvapi) ist auf diesem System nicht verfügbar. Die RTX-HDR-Einstellung wird voraussichtlich keine Wirkung haben.",
        ["rtx.applyFailedTitle"] = "RTX HDR nicht übernommen",
        ["rtx.applyFailedMsg"] = "Die RTX-HDR-Änderung wurde nicht ins NVIDIA-Treiberprofil geschrieben. Das Schreiben erfordert Administratorrechte – bitte die Windows-Nachfrage beim Speichern der Liste bestätigen.",
    ...
    private static readonly Dictionary<string, string> Fr = new()
    {
        ...
        ["wl.btn.addFile"] = "Choisir un fichier .exe…",
        ["wl.btn.addRunning"] = "Programme en cours…",
        ...
        ["entry.sec.hdrEnhance"] = "Amélioration HDR (jeux SDR)",
        ["entry.hdrEnhance.off"] = "Aucune",
        ["entry.hdrEnhance.autoHdr"] = "Auto HDR Windows",
        ["entry.hdrEnhance.rtx"] = "NVIDIA RTX HDR",
        ["entry.hdrEnhance.hint"] = "Convertit les jeux SDR en HDR. Auto HDR est défini dans les paramètres graphiques de Windows (chemin requis) ; si Auto HDR est désactivé globalement, HDR Pilot l'active pendant la session de jeu puis restaure l'état précédent. RTX HDR est défini dans le profil du pilote NVIDIA du jeu (GPU RTX requis, confirmation administrateur unique à l'enregistrement) ; le réglage fin reste dans l'application NVIDIA.",
        ["entry.rtxHdr.noNvidiaTitle"] = "Pilote NVIDIA introuvable",
        ["entry.rtxHdr.noNvidiaMsg"] = "L'interface du pilote NVIDIA (nvapi) n'est pas disponible sur ce système. Le réglage RTX HDR n'aura probablement aucun effet.",
        ["rtx.applyFailedTitle"] = "RTX HDR non appliqué",
        ["rtx.applyFailedMsg"] = "La modification RTX HDR n'a pas été écrite dans le profil du pilote NVIDIA. L'écriture nécessite des droits d'administrateur – veuillez confirmer l'invite Windows lors de l'enregistrement de la liste.",
    ...
    private static readonly Dictionary<string, string> Es = new()
    {
        ...
        ["wl.btn.addFile"] = "Elegir archivo .exe…",
        ["wl.btn.addRunning"] = "Programa en ejecución…",
        ["wl.btn.edit"] = "Editar…",
        ...
        ["entry.sec.hdrEnhance"] = "Mejora HDR (juegos SDR)",
        ["entry.hdrEnhance.off"] = "Ninguna",
        ["entry.hdrEnhance.autoHdr"] = "Auto HDR de Windows",
        ["entry.hdrEnhance.rtx"] = "NVIDIA RTX HDR",
        ["entry.hdrEnhance.hint"] = "Convierte juegos SDR a HDR. Auto HDR se establece en la configuración gráfica de Windows (requiere la ruta); si Auto HDR está desactivado globalmente, HDR Pilot lo activa durante la sesión de juego y luego restaura el estado anterior. RTX HDR se establece en el perfil del controlador NVIDIA del juego (requiere una GPU RTX y una confirmación de administrador al guardar); el ajuste fino queda en la aplicación NVIDIA.",
        ["entry.rtxHdr.noNvidiaTitle"] = "Controlador NVIDIA no encontrado",
        ["entry.rtxHdr.noNvidiaMsg"] = "La interfaz del controlador NVIDIA (nvapi) no está disponible en este sistema. El ajuste de RTX HDR probablemente no tendrá efecto.",
        ["rtx.applyFailedTitle"] = "RTX HDR no aplicado",
        ["rtx.applyFailedMsg"] = "El cambio de RTX HDR no se escribió en el perfil del controlador NVIDIA. Escribirlo requiere derechos de administrador: confirma el aviso de Windows al guardar la lista.",
